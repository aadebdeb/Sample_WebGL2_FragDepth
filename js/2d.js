...
`;
  ...
  const normalUniforms = getUniformLocations(gl, normalProgram, ['u_color']);
  const fragDepthUniforms = getUniformLocations(gl, fragDepthProgram, ['u_color', 'u_time']);

  const rectVao = createVao(gl,
    [{buffer: createVbo(gl, RECT_VERTICES_POSITION), size: 2, index: 0}],
